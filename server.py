...
def rand_string(): return ''.join(random.choices(string.ascii_uppercase +
                                                 string.ascii_lowercase + string.digits + '_', k=20))
...
def index():
    for fn in glob.glob(os.path.join(DIRNAME, '*')):
        os.remove(fn)
    assert len(glob.glob(os.path.join(DIRNAME, '*'))
               ) == 0, 'Found {} file'.format(len(glob.glob(os.path.join(DIRNAME, '*'))))
    if request.method == 'POST':
        ...
        for index, fn in enumerate(template_files):
            ...
            filename = os.path.join(
                DIRNAME, '{}_{}.jpg'.format(rand_string(), index))
